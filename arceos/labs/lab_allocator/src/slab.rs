...
impl<const BLK_SIZE: usize> Slab<BLK_SIZE> {
    ...
    pub fn allocate(
        ...
        match self.free_block_list.pop() {
            ...
            None => {
                let layout =                            
                    unsafe { Layout::from_size_align_unchecked(SET_SIZE * BLK_SIZE, 1) };//尽可能小
                    //info!("here");
                if let Ok(ptr) = tlsf.alloc(layout) {//试图从tlsf
                    unsafe {
                        self.grow(ptr.as_ptr() as usize, SET_SIZE * BLK_SIZE);//成功
                    }
    ...
    pub fn pop_free(&mut self)->Result<usize, AllocError> {
            match self.free_block_list.pop(){
                Some(block) => Ok(block.addr()),
                None =>Err(AllocError)
            }
    }
    pub fn is_free_empty(&mut self){
        if self.free_block_list.is_empty(){
            info!("empty")
        }
    }
